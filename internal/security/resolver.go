...
type LookupFunc func(ctx context.Context, network, server string) ([]netip.Addr, error)
...
func systemLookup(network, server string) (resolverName string, fn LookupFunc) {
	...
	return resolverName, func(ctx context.Context, network, _ string) ([]netip.Addr, error) {
		ips, err := r.LookupIP(ctx, network, "")
		if err != nil {
			return nil, err
		}
