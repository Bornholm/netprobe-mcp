...
func NewHTTPProberFromConfig(cfg config.HTTPProbeConfig, timeout time.Duration) *HTTPProber {
	return NewHTTPProber(HTTPProberConfig{
		...
		ReturnBody:        cfg.ReturnBody,
		ReturnHeaders:     cfg.ReturnHeaders,
		RequestHeaderList: cfg.RequestHeaderAllowList,
		AllowRequestBody:  cfg.AllowRequestBody,
		MaxRequestBody:    cfg.MaxRequestBodyBytes,
		UserAgent:         cfg.UserAgent,
		InsecureSkip:      cfg.InsecureSkipVerify,
		DisableCompress:   cfg.DisableCompression,
		AllowBodyRe:       cfg.AllowBodyRegexpChecks,
		BodyReTimeout:     cfg.BodyRegexpTimeout,
		MaxBodyReLength:   cfg.MaxBodyRegexpLength,
	}, timeout)
}
...
func (p *HTTPProber) Validate(opts *HTTPOptions) error {
	...
	// Enforce the body-regexp policy before the per-regex compile
	// loop: when AllowBodyRegexpChecks is off the whole pair of
	// arrays is rejected. This protects the agent from an
	// accidental regex enable.
	if (len(opts.FailIfBodyMatches) > 0 || len(opts.FailIfBodyNotMatches) > 0) && !p.cfg.AllowBodyRe {
		return &ErrInvalidRequest{Reason: "body regexp checks are disabled by policy (probes.http.allow_body_regexp_checks=false)"}
	}
	maxLen := p.cfg.MaxBodyReLength
	if maxLen <= 0 {
		maxLen = 256
	}
	for _, re := range opts.FailIfBodyMatches {
		if len(re) > maxLen {
			return &ErrInvalidRequest{Reason: "fail_if_body_matches regexp exceeds the configured length cap"}
		}
	...
	for _, re := range opts.FailIfBodyNotMatches {
		if len(re) > maxLen {
			return &ErrInvalidRequest{Reason: "fail_if_body_not_matches regexp exceeds the configured length cap"}
		}
		if _, err := regexp.Compile(re); err != nil {
