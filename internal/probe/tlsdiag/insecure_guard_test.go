...
// modules that need it. Anywhere else, a future contributor adding
// `InsecureSkipVerify: true` is a regression: a tool that diagnoses
// TLS security must not silently bypass certificate verification.
//
...
//   - handshake.go: the main handshake must surface the certificate
//     chain EVEN when Go's built-in verification rejects it
//     (hostname mismatch, unknown CA, expired intermediate). Without
//     InsecureSkipVerify, ConnectionState().PeerCertificates stays
//     empty on verification failure (verified experimentally, Go
//     1.25), making the diagnostic blind to a class of severe
//     misconfiguration. The flag is paired with VerifyPeerCertificate
//     which performs an equivalent strict verification (chain +
//     hostname) inside the analyser, so security is unchanged.
