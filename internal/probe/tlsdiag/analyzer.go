...
func (a *Analyzer) Diagnose(target *security.SafeTarget, opts DiagnoseOptions) (*Report, error) {
	...
	// Capture the verification error as a tentative verdict marker.
	// The chain is still inspected below so the report can list
	// concrete findings (TLS_HOSTNAME_MISMATCH, TLS_CHAIN_INCOMPLETE)
	// instead of an opaque "no certificates presented" message.
	var verifyErr error
	if hsErr != nil {
		verifyErr = hsErr
	}

	if hsErr != nil && len(outcome.ConnState.PeerCertificates) == 0 {
		// Handshake failed AND no certs were captured (rare: only
		// when the server closes the connection before responding to
		// our ClientHello). Surface that as a partial report.
	...
	// Inject a synthetic TLS_CHAIN_INCOMPLETE finding when the
	// verification callback returned an error AND no chain rule
	// has already fired. The native chain.go analysis already
	// produces TLS_HOSTNAME_MISMATCH for SNI issues, so we only
	// need to fill the gap for "unknown authority"-style failures
	// that produce no existing rule.
	if verifyErr != nil {
		alreadyFlagged := false
		for _, f := range rep.Findings {
			if f.ID == "TLS_HOSTNAME_MISMATCH" || f.ID == "TLS_CHAIN_INCOMPLETE" ||
				f.ID == "TLS_CERT_EXPIRED" || f.ID == "TLS_UNTRUSTED_ROOT" {
				alreadyFlagged = true
				break
			}
		}
		if !alreadyFlagged {
			rep.Findings = append(rep.Findings, Finding{
				...
				Category: "chain",
				Title:    "Handshake reported a verification error without a per-cert finding",
				Detail:   verifyErr.Error(),
				Remediation: "Investigate the chain trust anchor configuration; " +
					"this finding is reported when the cryptographic verification " +
					"fails without an existing rule pinpointing the cause.",
