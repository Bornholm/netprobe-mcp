...
- `internal/security` — Guard pipeline. `Guard.Authorize` returns the only `SafeTarget` value downstream may dial. `NormalizeHost` rejects non-canonical IP encodings and unqualified hostnames. `errs.DenyError` is the typed error shared across `security`, `ratelimit`, `mcpserver`.
...
make fuzz          # 10s fuzz of internal/security.NormalizeHost
make lint          # go vet ./...
make run           # ./bin/netprobe-mcp --config=configs/policy.example.yaml
```

CI runs the same plus `gofmt -l .` (fails on unformatted files) and a 30s `-fuzz=^FuzzNormalizeHost$` job. Release is `goreleaser` on `v*` tags; PRs run it in `--snapshot` mode.
